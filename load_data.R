...
#forwardbackward try from lora. will delete if doesn't work
...
library(data.table)
dt <- as.data.table(projectsundaymorning)
dt[,days:=format(Date,"%Y-%m-%d")]
dt[,.N,by=days]
...
Dec17T <- subset(train_data, Date=="2009-12-01")
Dec17Test <- subset(test1_data, Date=="2009-12-01")
Dec18T <- subset(train_data, Date=="2009-11-30")
set.seed(1)
...
testmod <- depmix(list(Global_active_power~1),
                  data=test1_data,
                  nstates=10,
                  family=list(gaussian())
)
...
de18 <- depmix(list(Global_active_power~1),
               data=Dec18T,
               nstates=10,
               family=list(gaussian())
)
testmodf <- setpars(testmod, getpars(fm, which="pars"), which="pars")
ans <- forwardbackward(testmodf, return.all=TRUE, useC=TRUE)
fm
ans

de18f <- setpars(de18, getpars(fm, which="pars"), which="pars")
sameyear <- forwardbackward(de18f, return.all=TRUE, useC=TRUE)
logLik(fm,method=c("fb","lystig","classification"),na.allow=TRUE)
